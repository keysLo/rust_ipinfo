...
| --- | --- |
| `1` (also accepts `true/yes/on`) | Restrict admin endpoints to `127.0.0.1` / `::1`; remote clients receive `403`. |
| `0` (also accepts `false/no/off`) or unset | Admin endpoints remain reachable from any client (default). The guard is bypassed without evaluating client IPs. |
